...
//playerController는 플레이어 캐릭터로서 Plater 게임 오브젝트를 제어함
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //마우스 왼쪽버튼을 눌렀으면  && 최대 점프 회수 (2)에 도달하지 않았으면 
        ...
        {
            //마우스 왼쪽 버튼에서 손을 뗀느 순간 && 속ㄷ의 y값이 양수라면(위로 상승 중)
            //현재 속도를 절반으로 변경
            playerRigidbody.linearVelocity = playerRigidbody.linearVelocity * 0.5f;
        }

        //애니메이터의 Grounded 피라미터를 isGrounded 값으로 변경
        animator.SetBool("Grounded", isGrounded);
    }

    private void Die()
    {
        //애니메이터의 Die 트리거 파라미털르 셋
        animator.SetTrigger("Die");

        // 오디오 소스에 할당된 오디오 클럽을 deathClip으로 변경
